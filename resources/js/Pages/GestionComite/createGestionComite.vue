...
const gestionComiteId = ref(0);
const showModal = ref(false);

onMounted(() => {
    obtenerCompromisos(gestionComiteId.value);
});

const form = useForm({
    fecha_comite: props.gestionComite.fecha_comite, comite_id: props.gestionComite.comite_id, estado: 1,
...
const save = () => {
    ...
    formData.append('id', gestionComiteId.value);
    ...
    axios.post(route('gestionComites.store'), formData, {
        headers: {
            'Content-Type': 'multipart/form-data',
        },
    }).then(response => {
        if (response.data.error == 0) {
            gestionComiteId.value = response.data.gestionComiteId;
...
<template>
    ...
    <AuthenticatedLayout :compromisos="props.compromisosN">
        <template #header>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <TextInput id="adjunto" class="form-control" name="adjunto" type="file" v-model="form.adjunto"
                                @change="handleFileChange"></TextInput>
            ...
            <ModalCompromiso :idGestion="gestionComiteId" :usuarios="props.usuarios" :fechaComite="form.fecha_comite"
                @compromisoGuardado="handleCompromisoGuardado"></ModalCompromiso>

            <ModalCompromisoDos :showModal="showModal" :idGestion="gestionComiteId" :usuarios="props.usuarios"
