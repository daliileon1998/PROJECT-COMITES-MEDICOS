...
const form = useForm({
    ...
    password2: props.usuario.password2, cargo: props.usuario.cargo, firma:props.usuario.firma,firma_url:props.usuario.firma_url
});

console.log("form ------->",form);

const update = () => {
    const fileInput = document.getElementById('firmaUpdate');
            ...
            if (fileName != "") {
                ...
                formData.append('firma', document.getElementById('firmaUpdate').files[0]);
                //document.getElementById('adjunto').files[0]
            } else {
                formData.append('firma', '');
            }

           axios.post(route('usuarios.update', form.id, { usuario: formData }), formData, {
                headers: {
    ...
    /*form.put(route('usuarios.update', id), {
        onSuccess: (response) => {
            mensaje();
        }
    });*/
...
<template>
    ...
    <AuthenticatedLayout :compromisos="props.compromisos">
        <template #header>
            <form @submit.prevent="update()" enctype="multipart/form-data" method="PUT">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <TextInput id="name" class="form-control" name="name" type="text" v-model="form.name"
                                maxlength="60" placeholder="Ingrese el Nombre del Usuario"></TextInput>
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <TextInput id="phone" class="form-control" name="phone" type="number" v-model="form.phone"
                                maxlength="10" placeholder="Ingrese el Telefono del Usuario"></TextInput>
                        </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <TextInput id="firmaUpdate" class="form-control" name="firmaUpdate"
                                        type="file" v-model="form.firma" accept="image/*"></TextInput>
                                    <div v-if="form.errors.firma" class="text-sm text-danger">
                                        {{ form.errors.firma }}
                                    </div>
                                    <a v-if="form.firma_url !== null && form.firma_url !== ''"
                                        :href="form.firma_url" target="_blank">Firma</a>
