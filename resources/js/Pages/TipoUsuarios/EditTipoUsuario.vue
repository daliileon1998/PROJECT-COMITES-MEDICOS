...
const form = useForm({
    id: props.tipoUsuario.id, nombre: props.tipoUsuario.nombre, codigo: props.tipoUsuario.codigo, estado: props.tipoUsuario.estado,
    permisos: props.permisos
});

const update = () => {
    var id = document.getElementById('id').value;
    form.put(route('tipoUsuarios.update', id), {
        onSuccess: (response) => {
            mensaje();
        }
    });
...
const getPermiso = (moduloId, tipo) => {
    const permiso = form.permisos[moduloId];
...
console.log('form.permisos:', form.permisos);
                ...
                <dataTable id="tipoUsuariosTable" class="table table-striped table-bordered" :options="{
                    ...
                    <tbody>
                        <tr v-for="(modulo, i) in props.modulos" :key="modulo.id">
                            ...
                            <td>{{ modulo.nombre }}</td>
                            <td>
                                <Checkbox :name="'agregar_' + modulo.id" :checked="getPermiso(i, 'agregar')"
                                    @change="updateCheckbox(i, 'agregar')" />
                            </td>
                            <td>
                                <Checkbox :name="'editar_' + modulo.id" :checked="getPermiso(i, 'editar')"
                                    @change="updateCheckbox(i, 'editar')" />
                            </td>
                            <td>
                                <Checkbox :name="'eliminar_' + modulo.id" :checked="getPermiso(i, 'eliminar')"
                                    @change="updateCheckbox(i, 'eliminar')" />
                            </td>
                            <td>
                                <Checkbox :name="'visualizar_' + modulo.id" :checked="getPermiso(i, 'visualizar')"
                                    @change="updateCheckbox(i, 'visualizar')" />
                            </td>
                        </tr>
                    </tbody>
                </dataTable><br>
